...
function mapToPdf(el: SigningElement, ctx: ExportContext, pdfPageWidth: number, pdfPageHeight: number) {
...
export async function exportAsPdf(elements: SigningElement[], ctx: ExportContext): Promise<Blob> {
  if (ctx.document.kind === 'pdf') {
    ...
    for (const el of elements.filter(e => e.page === ctx.pageIndex)) {
      ...
      switch (el.type) {
        case 'text': {
          const fontSize = (el.properties?.fontSize ?? 14) * (w / Math.max(1, el.size.width));
        ...
        case 'date': {
          const text = String(el.properties?.value ?? new Date().toISOString().slice(0, 10));
          const fontSize = Math.min(18, h * 0.7);
          const font = await getFont(el.properties?.fontFamily);
          page.drawText(text, { x: x + 2, y: y + h - fontSize - 2, size: fontSize, font, color: rgb(0, 0, 0) });
          break;
        }
        case 'checkbox': {
          ...
          if (el.properties?.checked) {
            // Draw an X
            page.drawLine({ start: { x, y }, end: { x: x + h, y: y + h }, color: rgb(0.1, 0.1, 0.1), thickness: 1.5 });
            page.drawLine({ start: { x: x + h, y }, end: { x, y: y + h }, color: rgb(0.1, 0.1, 0.1), thickness: 1.5 });
          }
          break;
        }
        case 'radio': {
          const r = Math.min(w, h) / 2;
          page.drawCircle({ x: x + r, y: y + r, size: r, borderColor: rgb(0.2, 0.2, 0.2), borderWidth: 1 });
          if (el.properties?.selected) {
            page.drawCircle({ x: x + r, y: y + r, size: r * 0.6, color: rgb(0.1, 0.1, 0.1) });
          }
          break;
        }
        case 'stamp': {
          page.drawRectangle({ x, y, width: w, height: h, borderColor: rgb(0.1, 0.6, 0.3), borderWidth: 2 });
          ...
          page.drawText(text, { x: tx, y: ty, size: fontSize, font, color: rgb(0.1, 0.5, 0.3) });
          break;
        }
        case 'signature': {
        ...
        case 'initials': {
          ...
          const font = await getFont(el.properties?.fontFamily);
          page.drawText(text, { x: x + 4, y: y + (h - fontSize) / 2, size: fontSize, font, color: rgb(0, 0, 0) });
          break;
        }
        default:
          break;
  ...
  page.drawImage(embedded, { x: 0, y: 0, width: ctx.stageWidth, height: ctx.stageHeight });
  const fontCache = new Map<string, any>();
  const getFont = async (name?: string) => {
    const key = pickFont(name);
    if (!fontCache.has(key)) fontCache.set(key, await pdfDoc.embedFont(key));
  ...
  for (const el of elements.filter(e => e.page === ctx.pageIndex)) {
    ...
    switch (el.type) {
      case 'text': {
        const font = await getFont(el.properties?.fontFamily);
        page.drawText(String(el.properties?.text ?? ''), { x: x + 2, y: y + h - 12, size: 12, font, color: rgb(0, 0, 0) });
        break;
      }
      case 'signature': {
        const dataUrl: string | undefined = el.properties?.dataUrl;
      ...
      case 'date': {
        const text = String(el.properties?.value ?? new Date().toISOString().slice(0, 10));
        const font = await getFont(el.properties?.fontFamily);
        page.drawText(text, { x: x + 2, y: y + h - 12, size: 12, font, color: rgb(0, 0, 0) });
        break;
      }
      case 'checkbox': {
        page.drawRectangle({ x, y, width: h, height: h, borderColor: rgb(0.2, 0.2, 0.2), borderWidth: 1 });
...
export async function exportAsImage(stageEl: HTMLElement, opts: ExportOptions): Promise<Blob> {
  const scale = (opts.dpi ?? 144) / 96; // default DPI: 144 for sharper output
  const canvas = await html2canvas(stageEl, { scale, backgroundColor: '#ffffff' });
  const type = opts.format === 'png' ? 'image/png' : 'image/jpeg';
  const blob: Blob = await new Promise((resolve) => canvas.toBlob((b) => resolve(b as Blob), type));
  return blob;
}
...
export async function exportFullPdf(
  ...
  if (document.kind === 'pdf') {
    ...
    pageSizes = pages.map(() => ({ width: fallbackStage?.width ?? 800, height: fallbackStage?.height ?? 1100 }));
  }
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    const page = document.kind === 'pdf' ? pdfDoc.getPage(i) : pdfDoc.getPages()[i];
    ...
    const ctx = { document, pageIndex, stageWidth: baseW, stageHeight: baseH } as ExportContext;

    for (const el of elements.filter(e => e.page === pageIndex)) {
      const { x, y, w, h } = mapToPdf(el, ctx, pw, ph);
      switch (el.type) {
        case 'text': {
          const fontSize = Math.max(6, Math.min(48, (el.properties?.fontSize ?? 14) * (w / Math.max(1, el.size.width))));
          page.drawText(String(el.properties?.text ?? ''), { x: x + 2, y: y + h - fontSize - 2, size: fontSize, font: helv, color: rgb(0.07, 0.09, 0.15) });
          break;
        }
        case 'date': {
          const text = String(el.properties?.value ?? new Date().toISOString().slice(0, 10));
          const fontSize = Math.min(18, h * 0.7);
          page.drawText(text, { x: x + 2, y: y + h - fontSize - 2, size: fontSize, font: helv, color: rgb(0, 0, 0) });
          break;
        }
        case 'checkbox': {
          page.drawRectangle({ x, y, width: h, height: h, borderColor: rgb(0.2, 0.2, 0.2), borderWidth: 1 });
        ...
        case 'radio': {
          const r = Math.min(w, h) / 2;
          page.drawCircle({ x: x + r, y: y + r, size: r, borderColor: rgb(0.2, 0.2, 0.2), borderWidth: 1 });
          if (el.properties?.selected) page.drawCircle({ x: x + r, y: y + r, size: r * 0.6, color: rgb(0.1, 0.1, 0.1) });
          break;
        }
        case 'stamp': {
          page.drawRectangle({ x, y, width: w, height: h, borderColor: rgb(0.1, 0.6, 0.3), borderWidth: 2 });
          ...
          const tx = x + (w - tw) / 2;
          const ty = y + (h - fontSize) / 2;
          page.drawText(text, { x: tx, y: ty, size: fontSize, font: helv, color: rgb(0.1, 0.5, 0.3) });
          break;
        }
        case 'signature': {
          const dataUrl: string | undefined = el.properties?.dataUrl;
        ...
        case 'initials': {
          const text = String(el.properties?.value ?? '');
          const fontSize = Math.min(24, h * 0.8);
          page.drawText(text, { x: x + 4, y: y + (h - fontSize) / 2, size: fontSize, font: helv, color: rgb(0, 0, 0) });
          break;
        }
        default:
          break;
